...
import { DescriptionSection } from '@/app/products/[slug]/DescriptionSection';
...
export function ProductDetailClient({ initialProduct, similar, popular, trending }: ProductDetailClientProps) {
    const [product, setProduct] = useState<Product>(initialProduct);
    const [selectedVariants, setSelectedVariants] = useState<ProductVariant[]>([]);
    const [adjustedPrice, setAdjustedPrice] = useState<number>(initialProduct.price);

    const synthesizedVariant = selectedVariants.length > 0 ? {
        id: selectedVariants.map(v => v.id).join('-'),
        label: selectedVariants.map(v => v.label).join(', '),
        priceAdjustment: adjustedPrice - product.price,
        type: 'composite'
    } : null;
                        ...
                        <div className="space-y-4">
                            ...
                            {product.variants && product.variants.length > 0 && (
                                <div className="bg-white border border-slate-100 rounded-[32px] p-6 shadow-sm">
                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Select Option</p>
                                    <VariantSelector 
                                        variants={product.variants} 
                                        basePrice={product.price} 
                                        onVariantSelect={(variants: any, newPrice: number) => {
                            ...
                            <div className="hidden lg:block mb-6">
                                <ProductAddToCart 
                                    product={product} 
                                    selectedVariant={synthesizedVariant} 
                                    priceOverride={adjustedPrice || product.price} 
                                />
                            </div>
